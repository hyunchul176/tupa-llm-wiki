...
KO
> 이 주제로 LeapSpace에 넣을, 500자 이내의 영어 질문을 만들어줘.

EN
> Draft an English query (under 500 characters) for this topic to run on LeapSpace.

→ [researcher.elsevier.com](https://researcher.elsevier.com/) 에서 돌려 답을 복사한다. 질문은 하나일 필요 없다 — 각도가 다른 질문을 여러 개 만들어 여러 채팅에서 물어보고 답을 모두 가져와도 된다(답엔 어차피 인용·DOI가 붙는다).
→ Run it on LeapSpace and copy the answer. You can ask several queries in separate chats and bring all answers back (answers already carry citations/DOIs).

**② 결과를 후보에 더하기 / Fold results into your candidates**

KO
> 이 LeapSpace 답에서 인용된 논문들을 내 후보 목록에 더해줘. 답: 〈여기에 붙여넣기〉

EN
> Add the papers cited in this LeapSpace answer to my candidate list. Answer: 〈paste here〉
