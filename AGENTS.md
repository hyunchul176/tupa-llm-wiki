...
- **강조 규칙**(guide·리뷰카드 공통): 강조 수단은 둘로 역할을 나눈다 — **볼드(`<b>`)는 짧은 부분**(단어·짧은 어구, 또는 목록 항목의 머리말/라벨)에, **하이라이트(`<span class="mark">`, 형광펜)는 긴 부분**(한 구절~한 문장의 핵심 강조)에 쓴다. 짧은 걸 형광펜 칠하거나 긴 문장을 통째로 볼드 처리하지 말 것. 단, **어두운 배경의 프롬프트 박스(`.ptext`) 안에서는 형광펜이 안 보이므로 볼드만** 쓴다.
